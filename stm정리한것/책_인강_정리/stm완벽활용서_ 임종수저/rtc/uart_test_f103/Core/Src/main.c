...
/* USER CODE BEGIN PFP */
void rtc_to_ascii(uint8_t value, uint8_t* result);
void hex_to_ascii(uint8_t value, uint8_t* result);
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t UART1RxBuf[13] = {0};
uint8_t UART1RxBuf1;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  /* ?ùº?ã®?? 250ms Í∞ÑÍ≤©?úºÎ°? ?îÑÎ°úÍ∑∏?û® ?àò?ñâ?ïòÍ≤? Ïß∞Ïùå */
	  if (Uart1RxReady==SET)
	  {
		 if (UART1RxBuf1==0x02 || gCmdRx1Info == true)
		 {
			gCmdRx1Info = true;
		 	...
		 	if(gCntRx1 == 12)
		 	{
		 		//Ï≤¥ÌÅ¨?Ñ¨ ?ôï?ù∏?ïòÍ≥? ?ã§Î•¥Î©¥ ?èêÍ∏∞Ìïò?äî ÏΩîÎìú?äî ?ïÑÏß? ?ûë?Ñ± ?ïà?ñà?ùå
		 		uint8_t newline[2] = {'\r','\n'};
		 		uint8_t ascii[2] = {0};
		 		uint8_t value;
		 		uint8_t* start = "start \r\n";
